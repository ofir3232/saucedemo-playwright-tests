...
test('Missing last name', { tag: '@regression' }, async ({ inventoryPage, cartPage, checkoutPage }) => {
    ...
    await test.step('Fill user info with missing last name', async () => {
        await checkoutPage.fillUserInfo(USER_INFO);
        await checkoutPage.clickOnContinue();
    });

    await test.step('Validate error message for last name', async () => {
        await checkoutPage.validateErrorMessage(CheckoutErrorType.EMPTY_LAST_NAME);
    });
});